...
├── WebsocketStack        - WebSocket API (Cognito JWT authorizer), real-time notifications
...
├── AuthStack             - Cognito User Pool + app client (publishes ids to SSM)
├── BackendStack          - Backend (ECS Fargate, HTTP API with JWT auth) + CloudFront SSE streaming
└── FrontendStack         - Frontend (CloudFront + S3); consumes backend via SSM, deploys independently
```

> The backend is **API-first**: any frontend (web, mobile, desktop, CLI) can
> consume it over standard HTTP + a Cognito JWT, with no AWS SDK or SigV4 on the
> client. See the [Backend API guide](docs/src/content/docs/en/api.md).
...
User queries hit the backend SSE proxy (`POST /chat/projects/{id}/invoke`), which invokes Bedrock Agent Core on the client's behalf and relays events as Server-Sent Events — the browser never calls Bedrock directly. The IDP Agent invokes tools via MCP Gateway, performing hybrid search with the Search Tool, graph traversal with the Graph Tool, and managing outputs with the Artifact Tool. Session history is persisted to S3 to maintain conversation context. This path is served via CloudFront → internal ALB (not API Gateway) to avoid the 30s HTTP API timeout on long generations.

```
User Query
  -> CloudFront (VPC origin) -> Backend (FastAPI) SSE proxy [JWT]
...
API Gateway HTTP (Cognito JWT Auth) connects through VPC Link to a Private ALB, then to ECS Fargate running FastAPI. Clients send a standard `Authorization: Bearer <token>` header; the backend also re-validates the JWT itself and derives user identity from the token claims. It handles all data access including project/document management, workflow queries, hybrid search, chat sessions, custom agents, knowledge graph, and artifact management.

```
API Gateway HTTP (Cognito JWT Auth)
...
| Cognito JWT auth (not IAM/SigV4) | API-first backend consumable by any frontend with a standard Bearer token; no AWS SDK on the client |
| Backend SSE proxy for AI chat | Hide Bedrock Agent Core behind the API; served via CloudFront VPC origin to bypass the 30s HTTP API timeout |
| Backend-issued presigned URLs (upload/download/voice) | Client never holds AWS credentials; the backend mediates all S3/Bedrock access |
| Separate Auth/Backend/Frontend stacks (SSM-wired) | Backend deploys and runs independently of any frontend |
...
| GET | `/projects/{id}/documents/{id}/download-url` | Presigned document download URL |
...
| POST | `/chat/projects/{id}/invoke` | AI chat — SSE stream (via `BackendStream` URL) |
| POST | `/voice/connection` | Issue presigned `wss://` URL for voice session |
| GET/POST/PUT/DELETE | `/projects/{id}/agents` | Custom agent CRUD |
| GET/DELETE | `/artifacts` | List / delete artifacts |
| GET | `/artifacts/{id}/download-url` | Presigned artifact download URL |
