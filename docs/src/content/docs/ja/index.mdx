...
+-- LanceServiceStack     - LanceDB Service（Rust）+ Tokaトークナイザー（Rust）
...
+-- AuthStack             - Cognito User Pool + アプリクライアント（ID を SSM に公開）
+-- BackendStack          - Backend（ECS Fargate、HTTP API JWT 認証）+ CloudFront SSE ストリーミング
'-- FrontendStack         - Frontend（CloudFront + S3）; SSM 経由でバックエンド参照、独立デプロイ
```
