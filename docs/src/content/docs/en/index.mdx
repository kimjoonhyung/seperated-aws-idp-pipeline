...
+-- AuthStack             - Cognito User Pool + app client (ids to SSM)
+-- BackendStack          - Backend (ECS Fargate, HTTP API JWT) + CloudFront SSE streaming
'-- FrontendStack         - Frontend (CloudFront + S3); consumes backend via SSM
```
