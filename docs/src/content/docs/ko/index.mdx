...
├── AuthStack             - Cognito User Pool + 앱 클라이언트 (ID를 SSM에 게시)
├── BackendStack          - Backend (ECS Fargate, HTTP API JWT 인증) + CloudFront SSE 스트리밍
└── FrontendStack         - Frontend (CloudFront + S3); SSM으로 백엔드 참조, 독립 배포
```
