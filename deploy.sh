...
echo ""
echo "  Stacks: Vpc, Neptune, Storage, Event, Bda, Ocr, Transcribe, LanceService,"
echo "          Workflow, Websocket, Worker, Mcp, Agent, Webcrawler,"
echo "          Auth, Backend, Frontend"
echo "==========================================================================="
echo ""
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --info)
            FRONTEND_DOMAIN=$(aws cloudformation describe-stacks --stack-name IDP-V2-Frontend \
...
# Resolve CloudFormation template: prefer the copy next to this script (so the
# locally-modified template is used), otherwise download from the repo.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "$SCRIPT_DIR/deploy-codebuild.yml" ]]; then
    TEMPLATE_FILE="$SCRIPT_DIR/deploy-codebuild.yml"
    echo ""
    echo "Using local CloudFormation template: $TEMPLATE_FILE"
else
