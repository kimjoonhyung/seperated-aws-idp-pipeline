...
async def delete_project(project_id: str, user: CurrentUser) -> DeleteProjectResponse:
    ...
    if config.session_storage_bucket_name:
        session_prefix = f"sessions/{user.user_id}/{project_id}/"
        with contextlib.suppress(Exception):
    ...
    if config.agent_storage_bucket_name:
        agent_prefix = f"{user.user_id}/{project_id}/agents/"
        with contextlib.suppress(Exception):
