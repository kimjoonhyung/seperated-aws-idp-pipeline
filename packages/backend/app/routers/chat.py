...
async def get_project_sessions(
    ...
    sessions = await cached_query_sessions(user.user_id, project_id)
...
def get_chat_history(project_id: str, session_id: str, user: CurrentUser) -> ChatHistoryResponse:
    ...
    s3_path = (
        f"s3://{bucket_name}/sessions/{user.user_id}/{project_id}/session_{session_id}/agents/*/messages/message_*.json"
    )
...
async def update_session(
    ...
    s3 = get_s3_client()
    key = f"sessions/{user.user_id}/{project_id}/session_{session_id}/session.json"
    ...
    await invalidate(CacheKey.session_list(user.user_id, project_id))
...
async def delete_session(project_id: str, session_id: str, user: CurrentUser) -> DeleteSessionResponse:
    ...
    prefix = f"sessions/{user.user_id}/{project_id}/session_{session_id}/"
