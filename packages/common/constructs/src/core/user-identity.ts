...
const STATIC_CALLBACK_URLS = ['http://localhost:4200', 'http://localhost:4300'];

/**
 * Creates a Cognito UserPool configured for JWT-based API access from any web
 * client. Callback URLs are static here; the frontend stack registers its
 * CloudFront domain cross-stack via {@link UserIdentity.registerCallbackUrl}.
 */
export class UserIdentity extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // Publish to SSM so the backend/websocket stacks can wire JWT authorizers.
    new StringParameter(this, 'UserPoolIdParam', {
      parameterName: SSM_KEYS.USER_POOL_ID,
      stringValue: this.userPool.userPoolId,
    });
    new StringParameter(this, 'UserPoolClientIdParam', {
      parameterName: SSM_KEYS.USER_POOL_CLIENT_ID,
