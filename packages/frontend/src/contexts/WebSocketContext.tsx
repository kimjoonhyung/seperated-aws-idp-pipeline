...
export function WebSocketProvider({ children }: PropsWithChildren) {
  const { websocketUrl } = useRuntimeConfig();
  const { user } = useAuth();
  ...
  const connect = useCallback(async () => {
    if (!websocketUrl || !user?.access_token) {
      return;
    }
    ...
    // JWT authorizer가 `token` 쿼리 파라미터로 Cognito access token을 검증한다.
    const url = `${websocketUrl}?token=${encodeURIComponent(user.access_token)}`;
